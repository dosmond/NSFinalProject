...
class PDMServer:
    ...
    def run(self):
        ...
        print("            |                                       |")
        print("            |      RSA-Diffie Hellman Exchange      |")
        ...
        first_hash = self.server.gen_hash(abmodp + wbmodp)
        second_hash = self.server.gen_hash(wbmodp + abmodp)

        print("            |      K[hash(2^ab mod p, 2^Wb mod p)]  |")
        print("            |<______________________________________|")
        print("            |                                       |")
        self.server.send_first_hash(conn, first_hash)
        print("            |     K[hash(2^bW mod p, 2^ab mod p)]   |")
        print("            |______________________________________>|")
        print("            |                                       |")
...
class PDMClient:
    ...
    def run(self):
        ...
        first_hash = self.client.gen_hash(abmodp + wbmodp)
        second_hash = self.client.gen_hash(wbmodp + abmodp)

        # Receive K[hash( 2^ab mod p, 2^Wb mod p)]
        ...
        # Send K[hash(2^bW mod p, 2^ab mod p)]
